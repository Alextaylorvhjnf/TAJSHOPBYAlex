...
/**
 * TEMPLATE · luxury-electronics — «Black Gold Lux HUD» (v25 full rewrite)
 * ---------------------------------------------------------------------
 * Rich black #0A0A0C stage with champagne-gold #D4AF37 / #C98A1B neon
 * lines, 1px LUMINOUS GRADIENT borders (the padding-mask trick) on
 * glass-black cards, and slow ceremonial motion only (fade / rise /
 * shimmer — nothing bouncy). Signature = gold serif-feel display
 * headlines, a shimmer sweep travelling across CTAs, and gold
 * edge-light that wakes on product-card hover.
 */
...
import {
  Crown, Star, Package, Check, ChevronLeft, Flame, TrendingUp,
  HelpCircle, BadgeCheck, Sparkles, Diamond, Clock, Megaphone,
  ArrowLeft, ShieldCheck, Timer,
} from "lucide-react";
...
/* thin luminous gold gradient border (1px) — the padding-mask trick */
[data-tpl="luxury-electronics"] .le-frame{position:relative;padding:1px;background:linear-gradient(160deg,rgba(212,175,55,.75),rgba(212,175,55,.12) 34%,rgba(201,138,27,.12) 62%,rgba(243,229,184,.6));border-radius:1.4rem;-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;transition:box-shadow .6s}
[data-tpl="luxury-electronics"] .le-frame:hover{box-shadow:0 0 34px -8px rgba(212,175,55,.45)}
...
@media (prefers-reduced-motion:reduce){
  ...
  [data-tpl="luxury-electronics"] .le-halo{animation:none!important}
...
html:not(.dark) [data-tpl="luxury-electronics"] .bg-\\[linear-gradient\\(to_left\\,transparent\\,\\#D4AF37\\,rgba\\(243\\,229\\,184\\,\\.9\\)\\,\\#C98A1B\\,transparent\\)\\]{background-image:linear-gradient(to left,transparent,rgba(168,132,22,.7),rgba(201,162,39,.75),rgba(140,106,18,.65),transparent)}
...
html:not(.dark) [data-tpl="luxury-electronics"] .text-\\[\\#D4AF37\\]\\/\\[0\\.06\\]{color:rgba(168,132,22,.12)}
...
/* ── black-gold product card with edge-light on hover ─────────────── */
    ...
    <article className={cn("le-card le-glass group relative flex h-full flex-col overflow-hidden rounded-[1.4rem]", !product.inStock && "grayscale-[0.5]")}>
      ...
      <div className="relative flex flex-1 flex-col border-t border-[#D4AF37]/15 p-5">
        ...
        <div className="mt-auto pt-5">
          ...
          <p className="le-gold-glow text-lg font-black tracking-wide tabular-nums">
...
/* ── ceremonial centered section header ───────────────────────────── */
function LuxHeader({ title, subtitle, href }: { title: string; subtitle?: string; href?: string }) {
  return (
    <div className="mb-10 text-center">
      <h2 className="le-grad-text text-2xl font-black tracking-[0.06em] md:text-3xl">{title}</h2>
      {subtitle && <p className="mt-2 text-[11px] tracking-[0.25em] text-[#F3E5B8]/45">{subtitle}</p>}
...
export function LuxuryElectronicsTemplate({ data }: { data: HomeData }) {
  ...
  return (
    <div data-template-chrome="1" data-tpl="luxury-electronics" className="isolate w-full bg-[#0A0A0C] text-white">
      ...
      {/* ═══ HERO — cinematic black-gold stage ═══ */}
      <section className="relative mx-auto w-full max-w-[1440px] px-4 pt-2" aria-labelledby="le-hero">
        <div className="le-frame relative isolate overflow-hidden rounded-[2.5rem] shadow-[0_40px_90px_-40px_rgba(0,0,0,.95)]">
          {heroSlide ? (
            <Link
              href={heroSlide.ctaUrl ?? (heroSlide.product ? `/products/${heroSlide.product.slug}` : "/products")}
              className="group relative block h-[420px] sm:h-[560px]"
              aria-label={heroSlide.title}
            >
              <SlideArt slide={heroSlide} sizes="96vw" priority className="object-cover transition-transform duration-[9000ms] ease-out group-hover:scale-[1.03]" />
              <span aria-hidden className="absolute inset-0 bg-[linear-gradient(to_top,#0A0A0C_f2,#0A0A0C_66_35%,rgba(10,10,12,.25)_75%,transparent)]" style={{ background: "var(--le-hero-ov)" }} />
              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center px-6 pb-14 text-center">
                <p className="le-glass mb-5 inline-flex items-center gap-2 rounded-full px-4 py-2 text-[11px] font-bold tracking-[0.2em] text-[#F3E5B8]">
                  <Crown className="h-3.5 w-3.5 text-[#D4AF37]" aria-hidden />
                  مجموعه ویژه {store.storeName}
                </p>
                <h1 id="le-hero" className="le-grad-text max-w-2xl text-3xl font-black leading-[1.45] tracking-wide sm:text-4xl md:text-5xl">
                  {heroSlide.title}
                </h1>
                {heroSlide.subtitle && (
                  <p className="mt-4 max-w-lg text-sm leading-8 text-[#F3E5B8]/60">{heroSlide.subtitle}</p>
                )}
                <span className="le-cta mt-8 inline-flex h-12 items-center gap-2 rounded-xl px-8 text-sm font-black">
                  {heroSlide.ctaText ?? "مشاهده مجموعه"}
                  <ArrowLeft className="h-4 w-4 transition-transform duration-500 group-hover:-translate-x-1" aria-hidden />
                </span>
              </div>
            </Link>
          ) : (
            <div className="relative flex h-[420px] flex-col items-center justify-center px-6 text-center sm:h-[560px]">
          ...
          <span aria-hidden className="absolute inset-x-0 bottom-0 h-0.5 bg-[linear-gradient(to_left,transparent,#D4AF37,rgba(243,229,184,.95),#C98A1B,transparent)]" />
        </div>
      </section>
      ...
      {/* ═══ EXTRA SLIDES DUO — cinematic gold-framed ═══ */}
      {extraSlides.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-label="اسلایدهای لوکس">
          <Reveal>
            <div className="grid gap-4 md:grid-cols-2">
              {extraSlides.map((s) => (
                <Link
                  ...
                  className="le-frame group relative block h-[240px] overflow-hidden rounded-[1.75rem] sm:h-[300px]"
                >
                  <SlideArt slide={s} sizes="(max-width: 768px) 92vw, 46vw" className="object-cover transition-transform duration-[900ms] group-hover:scale-[1.03]" />
                  <span aria-hidden className="absolute inset-0 bg-[linear-gradient(to_top,rgba(10,10,12,.92),rgba(10,10,12,.25)_55%,transparent)]" />
      ...
      {/* ═══ CATEGORIES — gold-framed tiles ═══ */}
      {data.categories.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-cats">
          <Reveal>
            <LuxHeader title="دسته‌بندی‌های لوکس" subtitle="گزیده گالری" href="/products" />
            <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-8">
              {data.categories.map((c) => (
                <Link key={c.id} href={`/products?category=${c.slug}`} className="le-frame group relative block overflow-hidden rounded-[1.4rem]">
                  <span className="relative block aspect-square w-full overflow-hidden rounded-[1.35rem] bg-[#0E0E12]">
                    {c.image ? (
                      <Image src={c.image} alt={c.name} fill sizes="(max-width: 640px) 30vw, 16vw" className="object-cover opacity-80 transition-all duration-700 group-hover:scale-105 group-hover:opacity-100" loading="lazy" />
                    ) : (
                      <span className="grid h-full place-items-center text-lg font-black text-[#D4AF37]/30">{c.name.charAt(0)}</span>
                    )}
                    <span aria-hidden className="absolute inset-0 bg-gradient-to-t from-[#0A0A0C]/85 via-transparent to-transparent" />
                  </span>
                  <span className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-0.5 p-2.5 text-center">
      ...
      {data.discounted.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-label="پیشنهادهای ویژه">
          <Reveal>
              ...
              <div className="relative bg-[#0C0C10] p-6 md:p-8">
                ...
                <div className="relative flex flex-col items-start gap-5 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-black tracking-[0.2em] text-[#D4AF37]">پیشنهادهای طلایی</p>
      ...
      {data.featured.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-featured">
          <Reveal>
            <LuxHeader title="گزیده ویترین" subtitle="انتخاب سرآشپز" href="/products" />
      ...
      {data.discounted.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-deals-grid">
          <Reveal>
            <LuxHeader title="مجموعه طلایی" subtitle="قیمت‌های ویژه" href="/products?discount=1" />
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
      ...
      {data.exclusive.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-exclusive">
          <Reveal>
            <LuxHeader title="انحصاری‌های تاج" subtitle="فقط برای شما" />
            <div className="grid gap-4 md:grid-cols-2">
                ...
                <article key={p.id} className="le-frame group relative overflow-hidden rounded-[1.75rem]">
                  <div className="relative flex flex-col bg-[#0C0C10] sm:flex-row">
                    <Link href={`/products/${p.slug}`} aria-label={p.name} className="relative block aspect-square w-full shrink-0 bg-[#0E0E12] sm:w-[46%]">
                      ...
                      <span className="le-gold-glow absolute start-4 top-4 rounded-full border border-[#D4AF37]/40 bg-[#0A0A0C]/85 px-3 py-1 text-[10px] font-black tracking-widest backdrop-blur">انحصاری</span>
                    </Link>
      ...
      {data.bestsellers.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-best">
          <Reveal>
            <LuxHeader title="محبوب‌ترین‌ها" subtitle="انتخاب مشتریان" />
      ...
      {data.newest.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-newest">
          <Reveal>
            <LuxHeader title="تازه‌رسیده‌ها" subtitle="جدیدترین‌های سالن" href="/products" />
      ...
      {/* ═══ SHOWCASES — cinematic full-bleed bands ═══ */}
      {data.showcases.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-label="نمایشگاه ویژه">
          <Reveal>
            <LuxHeader title="نمایشگاه ویژه" subtitle="پیشنهادهای نمایشگاهی" />
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              {data.showcases.slice(0, 4).map((s) => (
                <Link
                  key={s.id}
                  href={s.buttonUrl ?? (s.product ? `/products/${s.product.slug}` : "/products")}
                  className="le-frame group relative flex min-h-[240px] flex-col justify-end overflow-hidden rounded-[1.75rem]"
                >
                  ...
                  <span aria-hidden className="absolute inset-x-0 top-0 h-0.5 bg-[linear-gradient(to_left,transparent,#D4AF37,rgba(243,229,184,.9),#C98A1B,transparent)]" />
      ...
      <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-label="خدمات ویژه">
        <Reveal>
      ...
      {data.faq.length > 0 && (
        <section className="mx-auto w-full max-w-[1440px] px-4 pt-14" aria-labelledby="le-faq">
          <Reveal>
            <LuxHeader title="پرسش‌های متداول" subtitle="پاسخ‌های شفاف" />
